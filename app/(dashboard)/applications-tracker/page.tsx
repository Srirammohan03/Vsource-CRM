...
const KANBAN_COLUMNS = [
  ...
  },
  {
    ...
    label: "Documents",
    headerStyle:
      "border-yellow-300 dark:border-yellow-900/50 text-yellow-805 dark:text-yellow-300 bg-yellow-100/50 dark:bg-yellow-950/15",
    badgeStyle:
      "bg-yellow-100/90 text-yellow-950 border border-yellow-200 dark:bg-yellow-950 dark:text-yellow-300",
  },
  {
    id: "Applied",
    label: "Applied",
  ...
  },
  {
    ...
    headerStyle:
      "border-emerald-200 dark:border-emerald-900/50 text-emerald-800 dark:text-emerald-300 bg-emerald-50/60 dark:bg-emerald-950/15",
    badgeStyle:
  ...
  },
  {
    ...
    label: "Enrolled",
    headerStyle:
      "border-green-300 dark:border-green-900/50 text-green-800 dark:text-green-300 bg-green-50/60 dark:bg-green-950/15",
...
export default function ApplicationsTrackerPage() {
  const [students, setStudents] = useState<LocalStudent[]>(
    initialStudents as LocalStudent[],
  );
  ...
  const mapStageToKanban = (stage: string): string => {
    ...
    if (val === "offer received" || val === "deposit paid")
  ...
  const getStudentColorThemeKey = (
    ...
    const visaStat = (student.visaDetails?.visaStatus || "")
      .toLowerCase()
      .trim();
  ...
  const handleOnDrop = (e: React.DragEvent, targetStage: string) => {
    ...
    if (targetIndex < currentIndex) {
      setWarningMessage(
        `You cannot move "${student.name}" backward once they have reached a higher stage.`,
      );
      return;
    }
    ...
    if (targetIndex > currentIndex + 1) {
      setWarningMessage(
        `Please move "${student.name}" one status at a time. The next logical stage is "${KANBAN_ORDER[currentIndex + 1]}".`,
      );
      return;
    }
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-3 border-b border-slate-200 dark:border-slate-800">
        <div>
          <h4 className="text-xs font-black uppercase tracking-widest text-slate-400">
            Applications Pipeline Tracking Board
          </h4>
          <p className="text-xs text-slate-500 dark:text-slate-400">
            Manage enrollment journeys below. Drag cards forward or use buttons
            to move them step-by-step.
          </p>
        </div>
        ...
        {KANBAN_COLUMNS.map((col, colIndex) => {
          ...
          const columnStudents = students.filter((student) => {
            ...
            if (
              ...
              student.counsellor !== filterCounsellor
            )
              return false;
            if (
